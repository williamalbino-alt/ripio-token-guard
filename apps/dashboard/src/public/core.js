...
const fmtMonth = (m) => {
	...
	let s = d.toLocaleDateString(getLang() === 'es' ? 'es-AR' : 'pt-BR', {
		year: 'numeric',
		month: 'long',
	});
...
function tbl(h, rows) {
	let s =
		...
		h.map((x, i) => '<th class="' + (i > 0 ? 'n' : '') + '">' + x + '</th>').join('') +
		'</tr></thead><tbody>';
	s += rows
		.map(
			(r) =>
...
function openModal(t, c) {
	document.getElementById('mo-body').innerHTML = '<h3>' + t + '</h3>' + c;
...
function head(icon, title, sub) {
	return (
		'<div class="page-head"><div class="ico">' +
		ico(icon) +
		'</div><div><h2>' +
		title +
		'</h2><p>' +
		sub +
		'</p></div><div class="head-actions"><span class="refresh-ts" id="refresh-ts"></span><button class="btn-download" onclick="downloadPage()" title="' +
		t('core.downloadReport') +
		'"><i data-lucide="download"></i></button><button class="btn-refresh" onclick="refreshPage()" title="Refresh"><i data-lucide="refresh-cw"></i></button></div></div>'
	);
}
...
function updateRefreshTs() {
	...
	if (s < 5) el.textContent = getLang() === 'es' ? 'Justo ahora' : 'Agora mesmo';
	else if (s < 60) el.textContent = s + 's';
	else el.textContent = Math.floor(s / 60) + 'm';
}
...
async function refreshInBackground(url, key, cachedTs) {
	...
	try {
		...
		if (freshStr !== cachedStr) {
			...
			if (ov && ov.classList.contains('active') && url === '/api/summary') {
				loadOverview();
			}
		}
	} catch {}
...
function invalidateCache(url) {
	if (url) {
		localStorage.removeItem('tg_' + url);
		return;
	}
	// Invalidate all
	...
	es.onmessage = function (e) {
		try {
			...
			if (ev.type === 'alert') {
				showToast(
					t('core.alertPrefix') +
						ev.data.alertLevel.toUpperCase() +
						' — $' +
						ev.data.cost.toFixed(2),
					ev.data.alertLevel === 'critical' ? 'danger' : 'warning',
				);
			}
...
function showToast(msg, type) {
	...
	t.style.cssText =
		'position:fixed;bottom:20px;right:20px;padding:12px 20px;border-radius:12px;font-size:13px;font-weight:600;z-index:9999;animation:pageIn .3s ease;box-shadow:0 8px 32px rgba(0,0,0,.15);max-width:380px;';
	if (type === 'danger')
		t.style.cssText += 'background:#fee2e2;color:#991b1b;border:1px solid #fecaca;';
	else if (type === 'warning')
		t.style.cssText += 'background:#fef3c7;color:#92400e;border:1px solid #fde68a;';
...
async function downloadPage() {
	...
	overlay.innerHTML =
		'<div class="spinner-wrap" style="background:rgba(255,255,255,0.9);position:fixed;inset:0;z-index:99999;backdrop-filter:blur(4px);"><div class="spinner"></div><span class="spinner-text" style="font-size:16px;">' +
		t('core.generatingReport') +
		'</span></div>';
	...
	try {
		...
		const pages = Array.from(document.querySelectorAll('.page')).filter(
			(p) => p.id !== 'page-alerts' && p.id !== 'page-management',
		);
		...
		pages.forEach((p) => {
			pageHTML +=
				'<div style="margin-bottom:60px;padding-bottom:40px;border-bottom:2px dashed #e4e4e7;">' +
				p.innerHTML +
				'</div>';
		});
		...
		const now = new Date();
		const ts = now.toLocaleDateString(getLang() === 'es' ? 'es-AR' : 'pt-BR', {
			year: 'numeric',
			month: 'long',
			day: 'numeric',
			hour: '2-digit',
		...
		const fileName =
			'TokenGuard-' + pageName.replace(/[^a-zA-Z0-9]/g, '_') + '-' + fileTs + '.html';
